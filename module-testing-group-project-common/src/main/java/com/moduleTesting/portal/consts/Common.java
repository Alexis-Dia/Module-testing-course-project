package com.moduleTesting.portal.consts;

public interface Common {

    String ROLE_DRIVER = "DRIVER";
    String ROLE_ADMIN = "ADMIN";

    String MSG_ERR_EDITING_NOT_CURRENT_USER_IS_FORBIDDEN = "Editing not current user is forbidden.";
    String MSG_ERR_GETTING_NOT_CURRENT_USER_IS_FORBIDDEN = "Getting not current user is forbidden.";
    String MSG_ERR_USER_WASN_T_FOUND = "User wasn't found.";
    String MSG_ERR_USER_ALREADY_EXISTS = "User with the same email already exists.";
    String MSG_ERR_INCORRECT_PASSORD = "Password or username is incorrect.";
    String MSG_ERR_TASK_NOT_FOUND = "Task not found!";
    String MSG_ERR_CAR_NOT_FOUND = "Car not found!";
    String MSG_ERR_NOT_ENOUGH_POUNDS_ON_ADMIN_ACCOUNT = "Not enough pounds on admin account!";
    String MSG_ERR_FIRST_NAME_IS_NOT_VALID = "First name is not valid.";
    String MSG_ERR_LAST_NAME_IS_NOT_VALID = "Last name is not valid.";
    String MSG_ERR_BIRTHDAY_IS_NOT_VALID = "Birthday is not valid.";
    String MSG_ERR_EMAIL_IS_NOT_VALID = "Email is not valid.";
}
